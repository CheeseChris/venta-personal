...
export default function GestionStockClient() {
  ...
  stock: Number(String(row.stock || row['Stock'] || row['Cantidad'] || '0').replace(/,/g, '').replace(/-/g, '0').trim()) || 0,
  precio_unitario: Number(String(row.precio_unitario || row['Precio'] || row['Precio Unitario'] || '0').replace(/,/g, '').trim()) || 0,
